...
async function loadMessages() {
  try {
    const res = await fetchWithAuth(`${API_BASE}/party/${currentPartyId}/messages`);
    if (!res.ok) {
    ...
    if (!Array.isArray(messages)) {
      throw new Error('Failed to load messages: not an array');
    }
    ...
    chatBox.innerHTML = '<div class="placeholder-box">Could not load messages. Please try refreshing the page.</div>';
